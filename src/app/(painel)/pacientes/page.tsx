...
export default async function PacientesPage({ searchParams }: { searchParams: Promise<{ sucesso?: string; q?: string; page?: string }> }) {
  ...
  let request = supabase.from("pacientes").select("id,nome_completo,nome_social,cpf,cns,data_nascimento,telefone,email,sexo,cidade,uf,ativo,foto_path", { count: "exact" }).eq("ativo", true);
    ...
    <SectionPage eyebrow="Cadastros / Pacientes" title="Pacientes" description="Central de identificação administrativa dos pacientes da empresa autorizada." primaryActionLabel="Novo paciente" primaryActionHref="/pacientes/novo">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-900/[0.03]">
        <div className="space-y-4 border-b border-slate-100 p-5">
          ...
          <ListToolbar query={query} placeholder="Buscar paciente pelo nome..." />
        </div>
