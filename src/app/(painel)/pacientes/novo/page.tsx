...
export default async function NovoPacientePage({ searchParams }: { searchParams: Promise<{ erro?: string; retorno?: string }> }) {
  ...
  return <SectionPage eyebrow="Cadastros / Pacientes / Novo" title="Novo paciente" description={retornoAdmissao ? "Cadastre o paciente e retorne automaticamente para a admissão em andamento." : "Cadastro completo dividido por tipo de informação."}><form action={criarPaciente} noValidate className="ui-card p-5 sm:p-6">{retornoAdmissao ? <input type="hidden" name="retorno" value={retornoAdmissao} /> : null}{retornoAdmissao ? <div className="mb-5 rounded-xl border border-violet-200 bg-violet-50 px-4 py-3 text-sm text-violet-800"><strong>Admissão preservada.</strong> Ao salvar, este paciente será selecionado automaticamente na senha que estava em atendimento.</div> : null}{erro && mensagens[erro] ? <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{mensagens[erro]}</div> : null}<FormTabs tabs={[{ id: "pessoal", label: "Dados Pessoais", content: dadosPessoais }, { id: "contatos", label: "Contatos e Endereços", content: <ContactSections /> }]} /><div className="mt-8 flex flex-wrap justify-end gap-3 border-t border-slate-100 pt-5"><Link href={retornoAdmissao || "/pacientes"} className="btn-secondary">Cancelar</Link><button type="submit" className="ui-button-primary">{retornoAdmissao ? "Salvar e voltar à admissão" : "Salvar paciente"}</button></div></form></SectionPage>;
}
