...
function Campo({ label, name, type = "text", required = false, maxLength }: { label: string; name: string; type?: string; required?: boolean; maxLength?: number }) {
  return <label className="space-y-2 text-sm font-medium text-slate-700"><span>{label}{required ? " *" : ""}</span><input name={name} type={type} required={required} maxLength={maxLength} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 outline-none focus:border-brand-700" /></label>;
}
...
export default async function NovoPacientePage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  return (
    <SectionPage eyebrow="Cadastros / Pacientes / Novo" title="Novo paciente" description="Cadastro mestre administrativo do paciente, protegido por vínculo de empresa, permissão e RLS.">
      <form action={criarPaciente} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="space-y-2 text-sm font-medium text-slate-700"><span>Sexo</span><select name="sexo" defaultValue="nao_informado" className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5"><option value="nao_informado">Não informado</option><option value="feminino">Feminino</option><option value="masculino">Masculino</option><option value="intersexo">Intersexo</option></select></label>
        ...
        <div className="mt-8 flex justify-end gap-3 border-t border-slate-100 pt-5"><Link href="/pacientes" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">Cancelar</Link><button type="submit" className="rounded-lg bg-brand-950 px-4 py-2 text-sm font-semibold text-white">Salvar paciente</button></div>
      </form>
    </SectionPage>
  );
}
