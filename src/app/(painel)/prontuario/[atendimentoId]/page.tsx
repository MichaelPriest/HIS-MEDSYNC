...
import { Activity, BedDouble, ClipboardList, FileHeart, Pill, ShieldCheck, UserRound } from "lucide-react";
...
export default async function AtendimentoMedicoPage({ params }: { params: Promise<{ atendimentoId: string }> }) {
  ...
  const [{ data: atendimento }, { data: triagem }, { data: evolucoes }, { data: prescricoes }, { data: internacoes }] = await Promise.all([
    supabase.from("atendimentos").select("id,numero_atendimento,status,data_abertura,tipo_atendimento,origem,cobertura,numero_carteirinha,validade_carteirinha,numero_autorizacao,senha_autorizacao,paciente:pacientes(nome_completo,cpf,cns,ra,numero_registro,data_nascimento,sexo),profissional:profissionais(nome_completo,especialidade),convenio:convenios(nome_fantasia,razao_social),plano:convenio_planos(nome,codigo)").eq("id", atendimentoId).maybeSingle(),
