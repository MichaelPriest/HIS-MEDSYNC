...
import { assinarPrescricaoMedica, suspenderPrescricaoMedica } from "@/modules/prontuario-medico/prescricao-actions";
...
const errorMessages: Record<string, string> = {
  ...
  categoria: "Este tipo de item não pode ser lançado neste fluxo.",
...
export default async function PrescricaoMedicaPage({ params, searchParams }: {
  ...
  const [prescricoesRes, componentesRes, profissionalUsuarioRes, permissoes] = await Promise.all([
    supabase.from("prescricoes")
      .select("id,tipo,item,item_assistencial_id,quantidade,unidade_dose,dose,via,frequencia,duracao,horarios,instrucoes,orientacoes,se_necessario,status,assinado_em,suspenso_em,created_at,profissional_id,profissional:profissionais(nome_completo)")
  ...
  return (
    <SectionPage eyebrow="Assistencial / Atendimento médico / Prescrição" title={paciente?.nome_completo ?? "Paciente"} description={`Atendimento #${atendimento.numero_atendimento ?? "—"} · Registro #${paciente?.numero_registro ?? "—"} · ${paciente?.ra ?? "—"}`}>
      {sp.sucesso ? <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">{sp.sucesso === "assinada" ? "Prescrição assinada e ativada." : sp.sucesso === "suspensa" ? "Prescrição suspensa." : "Solicitação salva com sucesso."}</div> : null}
      ...
      {sp.aviso ? <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">A prescrição foi assinada, mas uma integração posterior precisa ser revisada.</div> : null}
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <div className="his-kpi"><p className="text-xs font-bold uppercase text-slate-400">Rascunhos</p><p className="mt-2 text-3xl font-black text-amber-700">{prescricoes.filter((i) => i.status === "rascunho").length}</p></div>
        <div className="his-kpi"><p className="text-xs font-bold uppercase text-slate-400">Ativas</p><p className="mt-2 text-3xl font-black text-emerald-700">{prescricoes.filter((i) => i.status === "ativa").length}</p></div>
      ...
      {profissional && canCreate ? <section className="his-card mt-5 p-5 sm:p-6"><div className="flex items-center gap-2"><Database className="size-5 text-brand-700"/><div><h2 className="font-black text-slate-900">Nova prescrição / solicitação</h2><p className="text-sm text-slate-500">Os campos mudam automaticamente conforme medicamento, material, exame ou procedimento.</p></div></div><PrescricaoDinamicaForm empresaId={empresaId} atendimentoId={atendimentoId}/></section> : null}

      <section className="mt-6 space-y-3">
        <div><h2 className="text-lg font-black text-slate-950">Prescrições do episódio</h2><p className="text-sm text-slate-500">Aqui você visualiza as prescrições. Prescrições assinadas podem ser abertas em formato de impressão.</p></div>
        {prescricoes.length ? prescricoes.map((item) => {
          ...
          const horarios = Array.isArray(item.horarios) ? item.horarios.join(", ") : "";
          return <article key={item.id} className="ui-card p-4 sm:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black uppercase text-slate-600">{item.tipo}</span><span className={`rounded-full px-2.5 py-1 text-xs font-black ${item.status === "ativa" ? "bg-emerald-100 text-emerald-800" : item.status === "suspensa" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}>{item.status}</span></div><h3 className="mt-2 font-black text-slate-950">{item.item}</h3><p className="mt-1 text-sm text-slate-600">{[item.dose,item.via,item.frequencia,item.duracao].filter(Boolean).join(" · ") || "Sem posologia complementar"}</p>{horarios ? <p className="mt-1 text-xs font-semibold text-brand-700">Horários: {horarios}</p> : null}{comps.length ? <div className="mt-3 rounded-xl bg-emerald-50 p-3"><p className="text-xs font-black uppercase text-emerald-700">Composição</p>{comps.map((comp) => { const ci=one(comp.item); return <p key={comp.id} className="mt-1 text-sm text-emerald-950">+ {ci?.descricao ?? "Componente"}{comp.dose ? ` · ${comp.dose}` : ""}{comp.quantidade ? ` · ${comp.quantidade} ${comp.unidade_dose ?? ""}` : ""}</p>; })}</div> : null}<p className="mt-2 flex items-center gap-1.5 text-xs text-slate-400"><Clock3 className="size-3.5"/>{fmtData(item.created_at)} · {prof?.nome_completo ?? "Profissional"}</p></div><div className="flex flex-wrap gap-2">{item.status === "rascunho" && canSign && item.item_assistencial_id ? <form action={assinarPrescricaoMedica}><input type="hidden" name="atendimento_id" value={atendimentoId}/><input type="hidden" name="prescricao_id" value={item.id}/><button className="ui-button-primary"><ShieldCheck className="size-4"/>Assinar</button></form> : null}{item.assinado_em ? <Link className="btn-secondary" href={`/prontuario/${atendimentoId}/prescricao/${item.id}/imprimir`}><Printer className="size-4"/>Visualizar / imprimir</Link> : null}{item.status === "ativa" && canSuspend ? <form action={suspenderPrescricaoMedica}><input type="hidden" name="atendimento_id" value={atendimentoId}/><input type="hidden" name="prescricao_id" value={item.id}/><input type="hidden" name="motivo" value="Suspensa pelo prescritor"/><button className="btn-secondary">Suspender</button></form> : null}</div></div></article>;
        }) : <div className="his-card p-8 text-center text-sm text-slate-500">Nenhuma prescrição neste atendimento.</div>}
      </section>
    </SectionPage>
  );
}
