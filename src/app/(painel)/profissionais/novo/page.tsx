...
const mensagens: Record<string, string> = {
  ...
  "habilitacao-tiss-incompleta": "Para habilitar o profissional no TISS, selecione CBO, conselho, UF e informe o número do conselho.",
  "uf-conselho-invalida": "A UF do conselho precisa ser uma UF brasileira válida.",
  "cbo-invalido": "Selecione um CBO válido da TUSS 24. O código 999999 não habilita cadastro mestre profissional.",
  "conselho-invalido": "Selecione um conselho profissional válido da TUSS 26.",
...
export default async function NovoProfissionalPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  if (user) {const { data: vinculo } = await supabase.from("usuario_empresas").select("empresa_id").eq("usuario_id", user.id).eq("ativo", true).limit(1).maybeSingle();empresaId = vinculo?.empresa_id ?? null;}

  const [{ data: tiposCatalogo }, { data: tiposLegados }, { data: cboRows }, { data: councilRows }] = await Promise.all([
    empresaId ? supabase.from("catalogos").select("id,codigo,descricao").eq("empresa_id", empresaId).eq("tipo", "tipo_profissional").eq("ativo", true).order("descricao") : Promise.resolve({ data: [] as Array<{ id: string; codigo: string; descricao: string }> }),
  ...
  const cboOptions = (cboRows ?? []).map((item) => ({ code: item.codigo, label: item.display }));
  const councilOptions = (councilRows ?? []).map((item) => ({ code: item.codigo, label: item.display }));
  const sourceVersion = cboRows?.[0]?.versao ?? councilRows?.[0]?.versao ?? "não informada";
  ...
  const profissional = <div className="space-y-5"><label className="block space-y-2 text-sm font-medium text-slate-700"><span>Tipo de Profissional *</span><select name="tipo_profissional_ref" required defaultValue="" className="ui-input"><option value="">Selecione o tipo</option>{temCatalogos ? tiposCatalogo?.map((tipo) => <option key={tipo.id} value={`catalogo:${tipo.id}`}>{tipo.descricao}</option>) : tiposLegados?.map((tipo) => <option key={tipo.id} value={`legado:${tipo.id}`}>{tipo.nome}</option>)}</select></label><ProfessionalRegulatoryFields cboOptions={cboOptions} councilOptions={councilOptions} sourceVersion={sourceVersion}/>{temCatalogos ? <p className="text-xs text-slate-500">O tipo profissional é empresarial; a habilitação regulatória usa os domínios ANS versionados.</p> : <p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">Nenhum Tipo de profissional empresarial foi encontrado. O sistema está exibindo os tipos legados. Cadastre novos tipos em Catálogos.</p>}</div>;
  ...
  return <SectionPage eyebrow="Cadastros / Profissionais / Novo" title="Novo profissional" description="Cadastro completo com dados pessoais, vínculo e habilitação regulatória padronizada quando houver uso TISS.">
    <FormJourney steps={[{label:"Identificação",description:"Dados pessoais, documentos e foto.",required:true},{label:"Habilitação",description:"CBO, conselho e UF em domínios oficiais quando houver uso TISS.",required:true},{label:"Contrato",description:"Vínculo, início, remuneração e carga horária.",required:true},{label:"Contatos",description:"Telefone, e-mail e endereço profissional.",required:true}]}/>
    <form noValidate action={criarProfissional} className="ui-card p-5 sm:p-6">{erro && mensagens[erro] ? <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{mensagens[erro]}</div> : null}<FormTabs tabs={[{ id: "pessoal", label: "Dados Pessoais", content: pessoal }, { id: "profissional", label: "Dados Profissionais", content: profissional }, { id: "contrato", label: "Contrato", content: contrato }, { id: "contatos", label: "Contatos e Endereços", content: <ContactSections /> }]} /><div className="mt-8 flex flex-wrap justify-end gap-3 border-t border-slate-100 pt-5"><Link href="/profissionais" className="btn-secondary">Cancelar</Link><button className="ui-button-primary">Salvar profissional</button></div></form>
  </SectionPage>;
}
