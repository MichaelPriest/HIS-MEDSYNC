...
export default async function NovoProfissionalPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  const profissional = <div><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3"><label className="space-y-2 text-sm font-medium text-slate-700"><span>Tipo de Profissional *</span><select name="tipo_profissional_id" required defaultValue="" className="ui-input"><option value="">Selecione o tipo</option>{tipos?.map((tipo) => <option key={tipo.id} value={tipo.id}>{tipo.nome}</option>)}</select></label><Campo label="Conselho" name="conselho" /><Campo label="Número do Conselho" name="numero_conselho" /><Campo label="UF do Conselho" name="uf_conselho" maxLength={2} /><Campo label="Especialidade" name="especialidade" /><Campo label="CBO" name="cbo" /></div></div>;
