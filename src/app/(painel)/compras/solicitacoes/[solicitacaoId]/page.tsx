...
export default async function SolicitacaoCompraPage({ params, searchParams }: {
  ...
  const { supabase, empresaId, unidadeId } = await requireAnyPermission([
    "compras.visualizar", "compras.solicitar", "compras.cotar", "compras.gerenciar",
  ]);
  const q = clean(String(sp.q ?? ""));
  const categoria = String(sp.categoria ?? "");
  const [{ data: solicitacao }, solicitarGrant, cotarGrant] = await Promise.all([
    supabase.from("compras_solicitacoes")
      .select("id,numero,setor,prioridade,status,justificativa,created_at,itens:compras_solicitacao_itens(id,item_assistencial_id,categoria_item,codigo_interno,tabela_tiss_codigo,codigo_tuss,codigo_tabela_propria,codigo_brasindice,codigo_simpro,codigo_anvisa,fabricante,apresentacao,descricao,quantidade,unidade_medida,observacoes)")
      .eq("id", solicitacaoId).eq("empresa_id", empresaId).eq("unidade_id", unidadeId).maybeSingle(),
  ...
  const canEdit = solicitarGrant.data === true && ["rascunho", "solicitada", "aprovada"].includes(solicitacao.status);
  const canQuote = cotarGrant.data === true || canEdit;
  const itensSelecionados = Array.isArray(solicitacao.itens) ? solicitacao.itens : [];

  let catalogQuery = supabase.from("itens_assistenciais")
    .select("id,codigo_interno,categoria,tabela_tiss_codigo,codigo_tuss,codigo_tabela_propria,descricao,unidade_medida,fabricante,apresentacao,codigo_anvisa,codigo_brasindice,codigo_simpro")
    .eq("empresa_id", empresaId).eq("ativo", true)
    .in("categoria", ["material", "medicamento", "opme", "gas_medicinal"])
    .order("descricao").limit(40);
  ...
  return <SectionPage
    eyebrow="Compras / Solicitação"
    title={`${solicitacao.numero} · ${solicitacao.setor || "Sem setor"}`}
    description="Monte a necessidade usando o catálogo mestre. Os códigos e dados regulatórios são copiados para a cotação como snapshot auditável."
