...
export default async function ImagemPage({ searchParams }: { searchParams: Promise<{ sucesso?: string; erro?: string }> }) {
  ...
  const [solReq, agendaReq, execReq, protoReq, laudoReq, equipReq] = await Promise.all([
    ...
    supabase.from("imagem_agendamentos").select("id,solicitacao_id,atendimento_id,protocolo_id,agendado_em,duracao_minutos,sala,equipamento,engenharia_equipamento_id,status,solicitacao:solicitacoes_exames(exame),atendimento:atendimentos(numero_atendimento,paciente:pacientes(nome_completo)),engenharia:engenharia_equipamentos(patrimonio,nome,status)").eq("unidade_id", unidadeId).order("agendado_em", { ascending: true }).limit(120),
