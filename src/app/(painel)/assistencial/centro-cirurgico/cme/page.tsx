...
import { salvarCicloCme } from "@/modules/centro-cirurgico/actions";
...
export default async function CmePage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return (
    ...
    >
      {params.sucesso ? <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700"><CheckCircle2 className="mr-2 inline size-4" />Ciclo CME salvo com sucesso.</div> : null}
...
function CicloForm({ ciclo }: { ciclo?: Ciclo }) {
  const indicador = ciclo?.indicadores ?? {};
  return <form action={salvarCicloCme} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
