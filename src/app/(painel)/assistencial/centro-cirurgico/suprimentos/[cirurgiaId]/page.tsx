...
import { Boxes, CheckCircle2, PackageCheck, RotateCcw, Scissors } from "lucide-react";
import { SurgicalSupplyBackgroundForm } from "@/components/centro-cirurgico/surgical-supply-background-form";
...
export default async function SuprimentosCirurgiaPage({ params }: { params: Promise<{ cirurgiaId: string }> }) {
  const { cirurgiaId } = await params;
  ...
  >
    ...
    <section className="his-card mt-5 p-5">
      ...
      <SurgicalSupplyBackgroundForm kind="request" className="mt-4 space-y-4">
    ...
    <section className="his-card mt-5 p-5">
      ...
      {!podeConsumir ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">A baixa física fica habilitada somente quando a cirurgia está <b>em andamento</b>. Status atual: {label(cirurgia.status)}.</div> : <SurgicalSupplyBackgroundForm kind="consume" className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <input type="hidden" name="cirurgia_id" value={cirurgia.id}/>
