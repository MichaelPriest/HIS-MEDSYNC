...
import {
  aplicarSlaInstitucionalEmergencia,
  atualizarRegistroEmergencia,
...
export default async function UrgenciaSlaPage({
  ...
  const [filaReq, configuracoesReq] = await Promise.all([
    supabase
    ...
    supabase
      ...
      .select("id,classificacao_risco,sla_minutos,referencia_institucional,vigente_desde")
      .eq("empresa_id", empresaId)
      .eq("unidade_id", unidadeId)
      .eq("ativo", true),
  ]);
  ...
  if (filaReq.error) {
    console.error("[urgencia] carregar fila SLA", { code: filaReq.error.code });
  }
  if (configuracoesReq.error) {
    console.error("[urgencia] carregar SLA institucional", { code: configuracoesReq.error.code });
  }

  const registros = ((filaReq.data ?? []) as FilaRegistro[]).sort((a, b) => {
  ...
  const configuracoesAtivas = new Map(
    ((configuracoesReq.data ?? []) as ConfiguracaoSlaAtiva[]).map((item) => [item.classificacao_risco, item]),
  );
    ...
    <main className="mx-auto max-w-7xl space-y-6 p-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link className="rounded-lg border px-4 py-2 text-sm font-medium hover:bg-slate-50" href="/assistencial/urgencia/sla/configuracoes">
      ...
      {params.sucesso === "sla-institucional-aplicado" ? (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">SLA institucional aplicado explicitamente ao atendimento.</div>
      ) : params.sucesso ? (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">Configuração do atendimento atualizada.</div>
      ) : null}
      ...
      {filaReq.error || configuracoesReq.error ? <div className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800">Falha ao carregar parte da fila operacional.</div> : null}
      ...
      <section className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
        A configuração institucional é uma referência vigente. Ela só altera um atendimento quando um usuário autorizado aciona <strong>Aplicar SLA institucional</strong>. Mudanças futuras da política não reescrevem atendimentos anteriores.
      </section>
      ...
      <section className="space-y-4">
        {registros.length === 0 ? (
          ...
          const institucional = registro.classificacao_risco ? configuracoesAtivas.get(registro.classificacao_risco) : undefined;
          return (
            <article key={registro.id} className={`rounded-xl border bg-white p-5 shadow-sm ${params.registro === registro.id ? "ring-2 ring-slate-200" : ""}`}>
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    Prioridade: {registro.prioridade ?? "—"} · SLA do atendimento: {registro.sla_minutos ? `${registro.sla_minutos} min` : "não configurado"} · Destino: {registro.destino ?? "—"}
                  </p>
                  <p className="mt-1 text-sm text-slate-600">
                    SLA institucional vigente: {institucional ? `${institucional.sla_minutos} min` : "não configurado para esta classificação"}
                    {institucional?.referencia_institucional ? ` · Referência: ${institucional.referencia_institucional}` : ""}
                  </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {institucional ? (
                    <form action={aplicarSlaInstitucionalEmergencia}>
                      <input type="hidden" name="emergencia_id" value={registro.id} />
                      <button className="rounded-lg border border-slate-900 px-3 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-50" type="submit">
                        Aplicar SLA institucional
                      </button>
                    </form>
                  ) : null}
                  <Link className="rounded-lg border px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" href={`/prontuario/${registro.atendimento_id}`}>Abrir prontuário</Link>
                </div>
              </div>
              ...
              <form action={atualizarRegistroEmergencia} className="mt-5 grid gap-3 border-t pt-5 md:grid-cols-2 lg:grid-cols-4">
                ...
                <label className="text-sm">SLA deste atendimento (min)
                  ...
                  <span className="mt-1 block text-xs text-slate-500">Ajuste manual do snapshot deste episódio; não altera o parâmetro institucional.</span>
                </label>
                ...
                <div className="flex items-end">
                  <button className="w-full rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800" type="submit">Atualizar atendimento</button>
                </div>
              </form>
            </article>
          );
