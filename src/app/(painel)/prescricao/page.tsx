...
export default async function PrescricaoPage({ searchParams }: { searchParams: Promise<{ sucesso?: string; erro?: string; atendimento?: string }> }) {
  ...
  const [{ data: atendimentos }, { data: profissionais }, { data: produtos }, { data: recentes }] = await Promise.all([
    ...
    supabase.from("prescricoes").select("id,item,dose,via,frequencia,status,assinado_em,requer_validacao_farmaceutica,created_at,atendimento:atendimentos(numero_atendimento,paciente:pacientes(nome_completo,ra,numero_registro)),profissional:profissionais(nome_completo)").order("created_at", { ascending: false }).limit(60),
  ]);
  ...
  return <SectionPage eyebrow="Assistência / Prescrição" title="Prescrição" description="Crie no contexto do episódio, assine, acompanhe validação e suspenda com rastreabilidade.">
    ...
    <section className="mt-4 grid gap-3 sm:grid-cols-3"><Kpi label="Rascunhos" value={rascunhos}/><Kpi label="Ativas" value={ativas}/><Kpi label="Aguardando validação" value={validacao}/></section>

    <div className="mt-5"><ActionPanel title="Novo item de prescrição" description="O item nasce como rascunho; medicamentos só seguem para a Farmácia após assinatura." defaultOpen={Boolean(params.atendimento)}><form action={criarPrescricao}>
      ...
      <div className="mt-5 flex justify-end border-t border-slate-200 pt-4"><button className="ui-button-primary">Salvar rascunho</button></div>
    </form></ActionPanel></div>

    <section className="his-card mt-5 overflow-hidden">
      <div className="border-b border-slate-100 p-5"><h2 className="font-black text-slate-900">Prescrições recentes</h2><p className="text-sm text-slate-500">Assine, acompanhe a validação e suspenda sem editar conteúdo já assinado.</p></div>
