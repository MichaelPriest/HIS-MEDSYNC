...
export default async function FaturamentoPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  return <SectionPage eyebrow="Ciclo da receita / Faturamento" title="Pré-faturamento" description="Abra a conta, trate críticas e siga para guias/lotes TISS sem sair do ciclo da receita.">
    ...
    <div className="mt-5"><ActionPanel title="Abrir pré-faturamento" description="Selecione o atendimento; a conta nasce vinculada ao episódio e segue o fluxo de auditoria/TISS."><form action={criarContaAtendimento}><EncounterPicker encounters={encounters} name="atendimento_id"/><div className="mt-4 flex justify-end"><button className="ui-button-primary">Abrir conta</button></div></form></ActionPanel></div>
