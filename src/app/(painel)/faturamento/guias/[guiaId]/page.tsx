import { AlertTriangle,CheckCircle2,FileCode2,RefreshCw,ShieldAlert,ShieldCheck } from "lucide-react";
...
function mensagemFalha(motivo?:string){
  if(motivo==="permissao") return "Seu perfil não possui permissão para revalidar esta guia.";
  if(motivo==="nao-localizada") return "A guia não foi localizada para validação.";
  return "Não foi possível concluir a validação. Os dados foram preservados; revise a guia e tente novamente.";
}

export default async function GuiaTissPage({ params,searchParams }: { params:Promise<{guiaId:string}>;searchParams:Promise<{erro?:string;validacao?:string;motivo?:string}> }){
  const { guiaId }=await params;
  const { erro,validacao,motivo }=await searchParams;
  ...
  return <SectionPage eyebrow="Financeiro / TISS / Guia" title={`Guia ${guia.numero_guia_prestador}`} description={`${paciente?.nome_completo??"Paciente"} · Registro #${paciente?.numero_registro??"—"} · ${paciente?.ra??"—"}`}>
    ...
    {validacao==="ok"?<div className="mb-4 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800"><CheckCircle2 className="mt-0.5 size-5 shrink-0"/><span>Validação concluída. A central de críticas abaixo mostra o estado atualizado da guia.</span></div>:null}
    {validacao==="erro"?<div className="mb-4 flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700"><ShieldAlert className="mt-0.5 size-5 shrink-0"/><span>{mensagemFalha(motivo)}</span></div>:null}
    ...
    <section className="ui-card mt-6 overflow-hidden">
      <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <form action={validarGuiaTiss}><input type="hidden" name="guia_id" value={guia.id}/><button type="submit" disabled={!podeRevalidar} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:bg-slate-300"><RefreshCw className="size-4"/>Revalidar guia</button></form>
        </div>
      </div>
