...
function isCommunicationGuideType(value:string):value is CommunicationGuideType{return value==="consulta"||value==="sp_sadt"||value==="resumo_internacao";}
...
export default async function GuiaTissPage({ params,searchParams }: { params:Promise<{guiaId:string}>;searchParams:Promise<{erro?:string}> }){
  ...
  const criticasAbertas:CriticaLinha[]=(criticas??[]).map((critica)=>({
    id:critica.id,item_id:critica.item_id,codigo:critica.codigo,severidade:critica.severidade==="alerta"?"alerta":"erro",campo:critica.campo,mensagem:critica.mensagem,created_at:critica.created_at,
  }));
  ...
  return <SectionPage eyebrow="Financeiro / TISS / Guia" title={`Guia ${guia.numero_guia_prestador}`} description={`${paciente?.nome_completo??"Paciente"} · Registro #${paciente?.numero_registro??"—"} · ${paciente?.ra??"—"}`}>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
      <Info label="Tipo de guia" value={guia.tipo_guia.replaceAll("_"," ")}/><Info label="Status" value={guia.status}/><Info label="Operadora" value={`${convenio?.nome_fantasia??"—"} · ANS ${guia.registro_ans??"—"}`}/><Info label="Carteirinha" value={guia.numero_carteirinha??"—"}/><Info label="Validação" value={validada?(resumo.erros?`${resumo.erros} bloqueio(s)`:"Sem bloqueios"):"Pendente"}/><Info label="Valor" value={`R$ ${Number(guia.valor_total??0).toLocaleString("pt-BR",{minimumFractionDigits:2})}`}/>
    </div>
    ...
    <section className="ui-card mt-6 overflow-hidden">
      <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-3"><span className={`grid size-11 shrink-0 place-items-center rounded-xl ${resumo.erros?"bg-rose-50 text-rose-700":resumo.alertas?"bg-amber-50 text-amber-700":"bg-emerald-50 text-emerald-700"}`}><ShieldCheck className="size-5"/></span><div><h2 className="font-semibold text-slate-900">Validação de consistência TISS</h2><p className="mt-1 max-w-3xl text-sm text-slate-600">Central de críticas para cadastro, autorização, domínios ANS/TUSS e requisitos estruturais 04.03.00. Esta pré-validação não substitui a validação do XML final pelo XSD.</p><p className="mt-2 text-xs text-slate-500">Última validação: {formatarValidacao(guia.validado_em)}</p></div></div>
        <div className="flex flex-wrap items-start gap-2"><span className="rounded-full bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700">{resumo.erros} bloqueio(s)</span><span className="rounded-full bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-700">{resumo.alertas} alerta(s)</span><GuideValidationBackgroundForm guiaId={guia.id} disabled={!podeRevalidar}/></div>
      </div>
    ...
    <section className="ui-card mt-6 p-5"><div className="flex items-start gap-3"><span className="grid size-10 place-items-center rounded-xl bg-amber-50 text-amber-700"><FileCode2 className="size-5"/></span><div><h2 className="font-semibold text-slate-900">Próxima barreira: XML final + XSD</h2><p className="mt-1 text-sm text-slate-600">A guia só entra em lote quando as críticas estruturais estão resolvidas. O lote final ainda precisa ser serializado como <code>mensagemTISS/loteGuias</code>, receber o MD5 regulatório no epílogo e passar pelo XSD oficial 04.03.00 antes de qualquer envio.</p></div></div></section>
  </SectionPage>;
}
