...
type Conta = {
  ...
  tipo_atendimento_faturamento: BillingType;
  tipo_atendimento_classificacao_origem: "automatico" | "manual";
  status: string;
...
const BILLING_TYPES: BillingType[] = ["pronto_atendimento", "ambulatorio", "internacao", "sadt"];
...
function TypeIcon({ type }: { type: BillingType }) {
  if (type === "internacao") return <Hospital className="size-4 text-brand-700" />;
  if (type === "pronto_atendimento") return <Stethoscope className="size-4 text-rose-600" />;
  if (type === "sadt") return <Activity className="size-4 text-violet-700" />;
  return <Building2 className="size-4 text-sky-700" />;
}
...
export default async function RelacaoContasPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { data, error } = await supabase
    ...
    .select("id,convenio_id,plano_id,competencia,tipo_cobranca,tipo_atendimento_faturamento,tipo_atendimento_classificacao_origem,status,valor_bruto,valor_desconto,valor_liquido,updated_at,paciente:pacientes(nome_completo,ra,numero_registro),atendimento:atendimentos(numero_atendimento,tipo_atendimento,data_abertura,data_fechamento,status),convenio:convenios(id,nome_fantasia),plano:convenio_planos(id,nome)")
  ...
  const tipoFiltro = BILLING_TYPES.includes((qs.tipo ?? "") as BillingType) ? (qs.tipo as BillingType) : "";
  ...
  const filtradas = contas.filter((conta) => {
    ...
    if (tipoFiltro && conta.tipo_atendimento_faturamento !== tipoFiltro) return false;
    ...
    if (!q) return true;
    const text = `${paciente?.nome_completo ?? ""} ${paciente?.ra ?? ""} ${paciente?.numero_registro ?? ""} ${atendimento?.numero_atendimento ?? ""} ${convenio?.nome_fantasia ?? ""} ${plano?.nome ?? ""} ${conta.competencia ?? ""} ${TYPE_LABELS[conta.tipo_atendimento_faturamento]}`.toLocaleLowerCase("pt-BR");
  ...
  const grupos = BILLING_TYPES.map((tipo) => {
    const rows = filtradas.filter((conta) => conta.tipo_atendimento_faturamento === tipo);
  ...
  return <SectionPage
    ...
    description="Contas classificadas e separadas em Pronto Atendimento, Ambulatório, Internação e SADT antes da geração de guias e lotes."
    ...
    <section className="his-card mt-5 overflow-hidden">
      <div className="border-b border-slate-100 p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div><p className="text-[10px] font-black uppercase tracking-[.14em] text-slate-400">Filtros avançados</p><h2 className="mt-1 font-black text-slate-950">Localizar contas sem misturar naturezas de atendimento</h2></div>
          <span className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-600">SADT corresponde ao exame eletivo fora do Pronto Atendimento e da Internação.</span>
        </div>
        <form className="grid gap-3 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-8">
          ...
          <select name="tipo" defaultValue={tipoFiltro} className="ui-input"><option value="">Todos os atendimentos</option>{BILLING_TYPES.map((tipo) => <option key={tipo} value={tipo}>{TYPE_LABELS[tipo]}</option>)}</select>
      ...
      <div className="grid gap-px border-b border-slate-100 bg-slate-100 sm:grid-cols-2 xl:grid-cols-4">
        {grupos.map((grupo) => <div key={grupo.tipo} className="bg-white px-5 py-4"><div className="flex items-center gap-2"><TypeIcon type={grupo.tipo} /><p className="text-xs font-black uppercase tracking-wide text-slate-500">{TYPE_LABELS[grupo.tipo]}</p></div><div className="mt-2 flex items-end justify-between gap-3"><strong className="text-xl text-slate-950">{grupo.count}</strong><span className="text-sm font-black text-slate-700">{money(grupo.total)}</span></div></div>)}
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtradas.length ? filtradas.map((conta) => {
              ...
              return <tr key={conta.id} className="align-top transition hover:bg-brand-50/35">
                ...
                <td className="px-4 py-3"><div className="flex items-center gap-2 font-black text-slate-800"><TypeIcon type={conta.tipo_atendimento_faturamento} />{TYPE_LABELS[conta.tipo_atendimento_faturamento]}</div><p className="mt-1 text-[11px] text-slate-500">Origem: {conta.tipo_atendimento_classificacao_origem === "manual" ? "revisada pelo faturamento" : "automática"}{atendimento?.tipo_atendimento ? ` · ${atendimento.tipo_atendimento}` : ""}</p></td>
