...
export default async function AtendimentosPage({ searchParams }: { searchParams: Promise<{ sucesso?: string; q?: string; status?: string }> }) {
  ...
  return <SectionPage eyebrow="Jornada / Atendimento" title="Atendimentos" description="O número do atendimento é também o número da guia do prestador e acompanha todo o episódio do paciente." primaryActionLabel="Abrir novo atendimento" primaryActionHref="/atendimentos/novo">
    ...
    <section className="ui-card overflow-hidden">
      <div className="space-y-4 border-b border-slate-100 p-5">
        <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-brand-50 text-brand-700"><ClipboardList className="size-5" /></span><div><h2 className="font-semibold text-slate-900">Fila e histórico</h2><p className="text-sm text-slate-500">Busque por atendimento/guia, paciente, CPF, CNS, RA ou registro e abra o prontuário integrado.</p></div></div>
        <ListToolbar query={query} placeholder="Buscar nº atendimento/guia, nome, CPF, CNS, RA ou registro..." />
