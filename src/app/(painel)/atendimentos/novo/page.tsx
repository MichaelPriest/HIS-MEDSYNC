...
export default async function NovoAtendimentoPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  const [{ data: pacientes }, { data: profissionais }, { data: tipos }, { data: convenios }, { data: planos }] = await Promise.all([
    supabase.from("pacientes").select("id,nome_completo,cpf,rg,data_nascimento,sexo,telefone,email,cep,logradouro,numero,complemento,bairro,cidade,uf,ra,numero_registro").eq("ativo", true).order("nome_completo").limit(1000),
  ...
  return <SectionPage eyebrow="Assistencial / Atendimento / Novo" title="Abrir atendimento" description="Admissão completa com confirmação dos dados do paciente, cobertura particular/convênio e dados do episódio assistencial.">
