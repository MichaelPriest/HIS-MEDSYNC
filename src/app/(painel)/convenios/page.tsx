...
function ansValido(value:string|null){return String(value??"").replace(/\D/g,"").length===6;}
...
export default async function ConveniosPage({ searchParams }: { searchParams: Promise<{ sucesso?: string;q?:string }> }) {
  ...
  const comAns=convenios.filter(c=>ansValido(c.registro_ans)).length;
  const ansPendentes=convenios.length-comAns;
