...
export function RecordDrawer({
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label={`Ver detalhes de ${title}`} className="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 transition hover:bg-slate-50 hover:text-slate-800">
        <MoreHorizontal className="size-4" />
      </button>
      {open ? (
        <div className="fixed inset-0 z-[60]">
          <button type="button" aria-label="Fechar detalhes" onClick={() => setOpen(false)} className="absolute inset-0 bg-slate-950/35 backdrop-blur-[2px]" />
          <aside className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto border-l border-slate-200 bg-white shadow-2xl">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/95 px-5 py-4 backdrop-blur">
              ...
              <button type="button" onClick={() => setOpen(false)} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100"><X className="size-5" /></button>
            </div>
            <div className="p-5">
              ...
              <dl className="mt-5 divide-y divide-slate-100 rounded-2xl border border-slate-200">
                {fields.map((field) => <div key={field.label} className="grid grid-cols-[8rem_1fr] gap-3 px-4 py-3"><dt className="text-xs font-medium text-slate-400">{field.label}</dt><dd className="break-words text-sm text-slate-700">{field.value || "—"}</dd></div>)}
              </dl>
