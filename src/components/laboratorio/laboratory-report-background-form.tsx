...
import {
  ...
  type LaboratoryReportActionState,
} from "@/modules/assistencial/laboratorio-laudo-actions";

type ReportActionKind = "save" | "validate" | "critical" | "release" | "rectify";
...
const INITIAL_STATE: LaboratoryReportActionState = { status: "idle" };
const actions = {
  save: salvarLaudoLaboratorio,
  validate: validarResultadoNoLaudo,
  critical: notificarCriticoNoLaudo,
  release: liberarLaudoLaboratorio,
  rectify: abrirRetificacaoLaudoLaboratorio,
} as const;

type ReportServerAction = (
  state: LaboratoryReportActionState,
  formData: FormData,
) => Promise<LaboratoryReportActionState>;

export function LaboratoryReportBackgroundForm({ kind, children, className, resetOnSuccess = false }: Props) {
...
export function OpenLaboratoryReportForm({ children, className }: { children: ReactNode; className?: string }) {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(abrirLaudoLaboratorio, INITIAL_STATE);
...
function ReportFeedback({ state, pending, compact = false }: { state: LaboratoryReportActionState; pending: boolean; compact?: boolean }) {
