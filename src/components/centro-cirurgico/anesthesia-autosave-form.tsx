...
export function AnesthesiaAutosaveForm({ cirurgiaId, initial }: { cirurgiaId: string; initial?: AnesthesiaInitialData | null }) {
  ...
  const [tecnica, setTecnica] = useState(initial?.tecnica ?? "");
  ...
  }), [asa, cirurgiaId, eventos, fluidos, medicamentos, monitor, observacoes, tecnica, viaAerea]);
  ...
  }, [asa, eventos, finalized, fluidos, medicamentos, monitor, observacoes, save, tecnica, viaAerea]);
  ...
  return <div className="mt-4 space-y-3">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Técnica anestésica</span><select value={tecnica} disabled={finalized} onChange={(e) => setTecnica(e.target.value)} className="ui-input"><option value="">Selecione</option>{techniques.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      {!initial?.inicio_em && !finalized ? <button type="button" onClick={() => void save(true, false)} disabled={state === "saving" || !tecnica} className="ui-button-primary"><Play className="size-4" />Iniciar anestesia</button> : null}
      {initial?.inicio_em && !finalized ? <button type="button" onClick={() => void save(false, true)} disabled={state === "saving" || !tecnica} className="rounded-xl bg-rose-700 px-4 py-2 text-sm font-black text-white hover:bg-rose-800"><Square className="mr-2 inline size-4" />Finalizar anestesia</button> : null}
      <span className="text-xs text-slate-500">Início {fmt(initial?.inicio_em)} · fim {fmt(initial?.fim_em)} · horário de Brasília</span>
    </div>
  </div>;
}
