...
import { Activity, Bell, BedDouble, BookOpenCheck, Building2, CalendarDays, ChevronDown, ChevronRight, ClipboardList, FlaskConical, FolderCog, HeartPulse, LayoutDashboard, Menu, MonitorCog, Pill, ReceiptText, ScanLine, Search, ShieldCheck, Stethoscope, TicketCheck, UserRound, UsersRound, X } from "lucide-react";
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <div className="flex h-full flex-col">
    ...
    <div className="flex-1 overflow-y-auto px-3 py-5">
      ...
      <div className="mt-7 border-t border-white/10 pt-5"><p className="px-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/35">Financeiro</p><div className="mt-3"><Link href="/faturamento" onClick={onNavigate} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${pathname.startsWith("/faturamento") ? "bg-white text-brand-950" : "text-white/65 hover:bg-white/10 hover:text-white"}`}><ReceiptText className="size-4"/><span>Faturamento / TISS</span></Link></div></div>
      <div className="mt-7 border-t border-white/10 pt-5"><p className="px-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/35">Configurações</p><div className="mt-3"><Link href="/configuracoes/paineis" onClick={onNavigate} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${pathname.startsWith("/configuracoes/paineis") ? "bg-white text-brand-950" : "text-white/65 hover:bg-white/10 hover:text-white"}`}><MonitorCog className="size-4"/><span>Painéis e chamadas</span></Link></div></div>
    </div>
...
export function AppShell({ children, email, logoutAction }: { children: React.ReactNode; email?: string | null; logoutAction: (formData: FormData) => void | Promise<void> }) {
  ...
  return <div className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[17.5rem_1fr]">
    ...
    <div className="min-w-0"><header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl"><div className="flex h-16 items-center gap-3 px-4 sm:px-6"><button onClick={() => setMobileOpen(true)} aria-label="Abrir menu" className="rounded-xl border border-slate-200 p-2 text-slate-600 hover:bg-slate-50 lg:hidden"><Menu className="size-5" /></button><div className="hidden min-w-0 flex-1 md:block"><div className="relative max-w-md"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input aria-label="Busca global" placeholder="Buscar pacientes, RA, registro, atendimentos..." className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm text-slate-700 outline-none transition focus:border-brand-300 focus:bg-white focus:ring-4 focus:ring-brand-100" /></div></div><div className="ml-auto flex items-center gap-2"><button aria-label="Notificações" className="relative rounded-xl border border-slate-200 bg-white p-2.5 text-slate-600 hover:bg-slate-50"><Bell className="size-4.5" /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-rose-500" /></button><details className="relative"><summary className="flex cursor-pointer list-none items-center gap-2 rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-sm hover:bg-slate-50"><span className="grid size-8 place-items-center rounded-full bg-brand-100 font-semibold text-brand-800">{email?.slice(0, 1).toUpperCase() || "U"}</span><span className="hidden max-w-40 truncate text-slate-700 sm:block">{email || "Usuário"}</span><ChevronDown className="size-4 text-slate-400" /></summary><div className="absolute right-0 mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-900/10"><Link href="/meu-perfil" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"><UserRound className="size-4" /> Meu Perfil</Link><div className="my-1 h-px bg-slate-100" /><form action={logoutAction}><button className="w-full rounded-xl px-3 py-2 text-left text-sm font-medium text-rose-600 hover:bg-rose-50">Sair do sistema</button></form></div></details></div></div></header><main className="px-4 py-5 sm:px-6 sm:py-6 xl:px-8">{children}</main></div>
  </div>;
}
