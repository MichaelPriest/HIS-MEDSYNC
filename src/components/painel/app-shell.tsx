...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const navLink = (item: NavItem) => {
    ...
    return <Link key={item.href} href={item.href as Route} onClick={onNavigate} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? "bg-white text-brand-950" : "text-white/65 hover:bg-white/10 hover:text-white"}`}><Icon className="size-4"/><span>{item.label}</span></Link>;
  };
