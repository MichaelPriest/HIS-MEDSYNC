  ...
  return (
    <div className="space-y-2 md:col-span-2 xl:col-span-3">
      ...
      <label className="space-y-2 text-sm font-medium text-slate-700">
        <span>Busca inteligente TUSS</span>
      ...
      {selected ? <div className="flex items-start justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm"><div><strong className="text-emerald-900">{selected.codigo}</strong><span className="ml-2 text-xs text-emerald-700">Tabela {selected.tabela}</span><p className="mt-1 text-emerald-800">{selected.descricao}</p></div><button type="button" onClick={()=>{setSelected(null);setQuery("");}} className="rounded-lg p-1.5 text-emerald-700 hover:bg-white"><X className="size-4" /></button></div> : code ? <div className="rounded-xl border border-brand-200 bg-brand-50 p-3 text-sm text-brand-900"><strong>{code}</strong><p className="mt-1 text-brand-800">{description || "Código padrão sugerido para o regime selecionado."}</p><p className="mt-1 text-xs text-brand-600">Pode ser substituído por item contratado/pacote na resolução contratual.</p></div> : null}
