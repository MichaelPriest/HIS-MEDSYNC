...
export function AdmissionForm({
  ...
  async function checkEligibility() {
    ...
    setEligibilityMessage("Canal da operadora configurado. A operação específica de elegibilidade ainda precisa estar parametrizada para executar a consulta online sem simular resposta.");
  }
  ...
  const profissional = <div className="space-y-5">
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Atendimento / Guia</p><p className="mt-1 font-semibold text-slate-800">O mesmo número será gerado automaticamente na abertura</p></div>
  ...
  const faturamento = <div className="space-y-5">
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-black text-emerald-950">Classificação do atendimento</p><p className="mt-1 text-xs text-emerald-800">Os códigos necessários ao faturamento são registrados automaticamente no episódio.</p></div>{ansVersion ? <span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-800">Configuração {ansVersion}</span> : null}</div>
    </div>
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      ...
      <label className="space-y-2 text-sm font-medium text-slate-700 xl:col-span-2"><span>Tipo de atendimento{coverage === "convenio" ? " *" : ""}</span><select name="tipo_atendimento_tuss50_codigo" value={tipoAns50} onChange={(event) => { setTipoAns50(event.target.value); if (event.target.value !== "04") setTipoConsultaAns52(""); }} className="ui-input"><option value="">Selecione</option>{tiposAtendimentoAns.map((item) => <option key={item.codigo} value={item.codigo}>{item.codigo} — {item.display}</option>)}</select>{tipoTiss === "sadt_exames" && !tipoAns50 ? <span className="block text-xs font-semibold text-amber-700">Escolha “Exame Ambulatorial” ou “Exame” conforme a regra do atendimento; o sistema não presume esta diferença.</span> : null}</label>
      {tipoAns50 === "04" ? <label className="space-y-2 text-sm font-medium text-slate-700"><span>Tipo de consulta{coverage === "convenio" ? " *" : ""}</span><select name="tipo_consulta_tuss52_codigo" value={tipoConsultaAns52} onChange={(event) => setTipoConsultaAns52(event.target.value)} className="ui-input"><option value="">Selecione</option>{tiposConsultaAns.map((item) => <option key={item.codigo} value={item.codigo}>{item.codigo} — {item.display}</option>)}</select></label> : <input type="hidden" name="tipo_consulta_tuss52_codigo" value="" />}
      {selectedAns50 ? <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600"><p className="font-bold text-slate-800">Classificação · {selectedAns50.codigo}</p><p className="mt-1">{selectedAns50.display}</p></div> : null}
  ...
  return <form noValidate action={action} className="ui-card p-5 sm:p-6">
    <div className="mb-5 grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-brand-100 bg-brand-50 p-3"><UserPlus className="size-4 text-brand-700"/><p className="mt-1 text-xs font-bold text-brand-900">1. Paciente</p><p className="text-[11px] text-brand-700">Identificação e alertas</p></div><div className="rounded-xl border border-indigo-100 bg-indigo-50 p-3"><Building2 className="size-4 text-indigo-700"/><p className="mt-1 text-xs font-bold text-indigo-900">2. Cobertura</p><p className="text-[11px] text-indigo-700">Plano, carteira e autorização</p></div><div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3"><Stethoscope className="size-4 text-emerald-700"/><p className="mt-1 text-xs font-bold text-emerald-900">3. Atendimento</p><p className="text-[11px] text-emerald-700">Profissional, procedimento e validações</p></div></div>
    <FormTabs tabs={[{ id: "paciente", label: "Paciente", content: dadosPaciente }, { id: "endereco", label: "Endereço", content: endereco }, { id: "cobertura", label: "Cobertura / Autorização", content: cobertura }, { id: "profissional", label: "Profissional", content: profissional }, { id: "faturamento", label: "Atendimento / Faturamento", content: faturamento }]} />
    <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5"><div className="flex items-center gap-2 text-xs text-slate-500"><Clock3 className="size-4" />Dados necessários ao faturamento serão registrados no momento da abertura.</div><div className="flex gap-3"><a href={cancelHref} className="btn-secondary">Cancelar</a><button disabled={!patient || isExpired || !ansReady} className="ui-button-primary disabled:cursor-not-allowed disabled:opacity-50">{submitLabel}</button></div></div>
  </form>;
}