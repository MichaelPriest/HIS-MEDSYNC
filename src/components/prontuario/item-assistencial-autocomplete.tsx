...
export type ItemAssistencialSelecionado = {
  id: string;
...
export function ItemAssistencialAutocomplete({
  empresaId,
  name = "item_assistencial_id",
  ...
  const [selecionado, setSelecionado] = useState<ItemAssistencialSelecionado | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const unicos = new Map<string, ItemAssistencialSelecionado>();
      for (const item of [...(porDescricao.data ?? []), ...(porCodigo.data ?? []), ...(porTuss.data ?? [])] as ItemAssistencialSelecionado[]) {
  ...
  return (
    <div className="relative">
      ...
      <label className="block space-y-2 text-sm font-semibold text-slate-700">
        ...
        <span className="relative block">
          ...
          <input
            ...
            onChange={(event) => {
              if (selecionado) onSelecionado?.(null);
              setSelecionado(null);
          ...
          />
          {(selecionado || termo) ? (
            <button type="button" aria-label="Limpar item" onClick={() => escolher(null)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700">
              <X className="size-4" />
            </button>
          ) : null}
        </span>
      ...
      {!selecionado && termo.trim().length >= 2 ? (
        <div className="absolute z-40 mt-1 max-h-80 w-full overflow-auto rounded-xl border border-slate-200 bg-white p-1 shadow-xl">
          ...
          {resultados.map((item) => (
            <button key={item.id} type="button" onClick={() => escolher(item)} className="block w-full rounded-lg px-3 py-2.5 text-left hover:bg-slate-50">
      ...
      {selecionado ? (
        <p className="mt-2 text-xs font-semibold text-emerald-700">
          {categoriaLabel[selecionado.categoria] ?? selecionado.categoria} selecionado do catálogo institucional.
        </p>
      ) : null}
    </div>
  );
