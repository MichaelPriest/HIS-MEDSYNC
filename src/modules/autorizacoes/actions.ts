...
export async function registrarIdentificacaoAutorizacao(formData: FormData) {
  ...
  const { data: autorizacao } = await supabase.from("autorizacoes_atendimento")
  ...
  const { data: config } = await supabase.from("convenio_identificacao_config")
  ...
  if (config?.exige_na_autorizacao && !metodoPermitido(config.metodo, metodo)) {
    redirect(`/autorizacoes?atendimento=${autorizacao.atendimento_id}&erro=metodo-identificacao`);
  }
