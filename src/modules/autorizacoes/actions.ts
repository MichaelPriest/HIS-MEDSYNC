...
export async function registrarIdentificacaoAutorizacao(formData: FormData) {
  ...
  const { data: autorizacao } = await supabase.from("autorizacoes_atendimento").select("id,atendimento_id,paciente_id,convenio_id")
    .eq("id", autorizacaoId).eq("unidade_id", unidadeId).maybeSingle();
  if (!autorizacao?.paciente_id || !autorizacao.convenio_id) redirect("/autorizacoes?erro=identificacao-contexto");

  const { data: config } = await supabase.from("convenio_identificacao_config").select("metodo,provedor,exige_na_autorizacao,ativo")
  ...
  const { error } = await supabase.from("autorizacao_identificacao_eventos").insert({
    empresa_id: empresaId, unidade_id: unidadeId, autorizacao_id: autorizacao.id, atendimento_id: autorizacao.atendimento_id,
    paciente_id: autorizacao.paciente_id, convenio_id: autorizacao.convenio_id, metodo,
    referencia_hash: hashRef(referencia), provedor: provedor ?? config?.provedor ?? null, dispositivo,
    validado: true, validado_em: new Date().toISOString(), created_by: user.id,
  });
...
export async function atualizarAutorizacao(formData: FormData) {
  ...
  const { data: autorizacao } = await supabase.from("autorizacoes_atendimento").select("id,atendimento_id,paciente_id,convenio_id")
    .eq("id", id).eq("unidade_id", unidadeId).maybeSingle();
  if (!autorizacao) redirect("/autorizacoes?erro=nao-encontrada");

  if (status === "autorizada" && autorizacao.convenio_id) {
    const { data: config } = await supabase.from("convenio_identificacao_config").select("metodo,exige_na_autorizacao,ativo")
      .eq("empresa_id", empresaId).eq("convenio_id", autorizacao.convenio_id).eq("ativo", true).maybeSingle();
  ...
  if (status === "autorizada" || status === "dispensada") {
    await supabase.from("atendimentos").update({ numero_autorizacao: payload.numero_guia_operadora, senha_autorizacao: payload.senha_autorizacao, updated_by: user.id, updated_at: new Date().toISOString() })
      .eq("id", autorizacao.atendimento_id).eq("unidade_id", unidadeId);

    const { data: atendimento } = await supabase.from("atendimentos").select("id,paciente_id,tipo_atendimento,especialidade_destino,triagem_concluida_em")
      .eq("id", autorizacao.atendimento_id).eq("unidade_id", unidadeId).maybeSingle();
    if (atendimento?.triagem_concluida_em && atendimento.especialidade_destino) {
      ...
      try {
        const destino = await encaminharPosTriagem({
          supabase, userId: user.id, empresaId, unidadeId, atendimentoId: atendimento.id, pacienteId: atendimento.paciente_id,
          tipoAtendimento: atendimento.tipo_atendimento, especialidade: atendimento.especialidade_destino,
          classificacao: triagem?.classificacao_risco ?? null, queixaPrincipal: triagem?.queixa_principal ?? null,
        });
        revalidatePath("/autorizacoes"); revalidatePath("/fila-medica"); revalidatePath("/pronto-socorro"); revalidatePath("/assistencial/urgencia");
