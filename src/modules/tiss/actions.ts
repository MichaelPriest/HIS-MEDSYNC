...
export async function criarLoteTiss(formData: FormData) {
  ...
  const { data: lote, error } = await supabase.from("tiss_lotes").insert({ empresa_id: empresaId, unidade_id: unidadeId, convenio_id: convenioId, versao_id: versao.id, numero_lote: numeroLote, competencia, previsao_pagamento: previsaoPagamento, status: "rascunho", quantidade_guias: elegiveis.length, valor_total: valorTotal, created_by: user.id }).select("id").single();
  ...
  await supabase.from("financeiro_recebiveis").insert({ empresa_id:empresaId, unidade_id:unidadeId, lote_id:lote.id, convenio_id:convenioId, competencia, previsao_pagamento:previsaoPagamento, valor_bruto:valorTotal, valor_liquido_previsto:valorTotal, status:"previsto", created_by:user.id, updated_by:user.id });
...
export async function registrarProtocolo(loteId: string, formData: FormData) {
  const { supabase, user, empresaId, unidadeId } = await getAssistencialContext(); const numero = text(formData,"numero_protocolo"); if (!numero) redirect(`/faturamento/lotes/${loteId}?erro=protocolo`);
  ...
  if (error) redirect(`/faturamento/lotes/${loteId}?erro=protocolo`); await supabase.from("tiss_lotes").update({ status:"protocolado", protocolo_operadora:numero, retorno_em:new Date().toISOString() }).eq("id",loteId); revalidatePath(`/faturamento/lotes/${loteId}`);
}

export async function registrarGlosa(loteId: string, formData: FormData) {
  const { supabase, empresaId, unidadeId } = await getAssistencialContext(); const codigo=text(formData,"codigo_glosa"); const valor=money(formData,"valor_glosado"); if (!codigo || valor <= 0) redirect(`/faturamento/lotes/${loteId}?erro=glosa`);
  const { error } = await supabase.from("tiss_glosas").insert({ empresa_id:empresaId, unidade_id:unidadeId, lote_id:loteId, protocolo_id:text(formData,"protocolo_id")||null, guia_id:text(formData,"guia_id")||null, guia_item_id:text(formData,"guia_item_id")||null, codigo_glosa:codigo, descricao_glosa:text(formData,"descricao_glosa")||null, valor_glosado:valor }); if (error) redirect(`/faturamento/lotes/${loteId}?erro=glosa`); revalidatePath(`/faturamento/lotes/${loteId}`);
}

export async function criarRecursoGlosa(formData: FormData) {
  const { supabase, user, empresaId, unidadeId } = await getAssistencialContext(); const glosaId=text(formData,"glosa_id"); const justificativa=text(formData,"justificativa"); const valor=money(formData,"valor_recursado"); if (!glosaId || !justificativa || valor<=0) redirect("/faturamento/glosas?erro=campos");
  const { data: glosa } = await supabase.from("tiss_glosas").select("id,valor_glosado,protocolo_id,lote:tiss_lotes(convenio_id)").eq("id",glosaId).maybeSingle(); if (!glosa || valor > Number(glosa.valor_glosado||0)) redirect("/faturamento/glosas?erro=valor");
  const lote = Array.isArray(glosa.lote) ? glosa.lote[0] : glosa.lote; if (!lote?.convenio_id) redirect("/faturamento/glosas?erro=convenio");
  const numero=`R${new Date().toISOString().slice(0,10).replaceAll("-","")}${String(Date.now()).slice(-6)}`;
  const { data: recurso, error } = await supabase.from("tiss_recursos_glosa").insert({ empresa_id:empresaId, unidade_id:unidadeId, convenio_id:lote.convenio_id, protocolo_id:glosa.protocolo_id, numero_recurso:numero, status:"rascunho", valor_total_recursado:valor, created_by:user.id, updated_by:user.id }).select("id").single(); if (error || !recurso) redirect("/faturamento/glosas?erro=recurso");
  const { error: itemError } = await supabase.from("tiss_recurso_itens").insert({ recurso_id:recurso.id, glosa_id:glosaId, valor_recursado:valor, justificativa }); if (itemError) redirect("/faturamento/glosas?erro=recurso-item"); await supabase.from("tiss_glosas").update({ status:"em_recurso" }).eq("id",glosaId); redirect(`/faturamento/recursos/${recurso.id}`);
}
