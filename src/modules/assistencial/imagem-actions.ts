...
export async function concluirExecucaoImagem(formData: FormData) {
  ...
  const { data: exec, error } = await supabase.from("imagem_execucoes").update({
    status: "concluido", finalizado_em: new Date().toISOString(), executado_por: profissionalId,
...
export async function registrarContrasteImagem(formData: FormData) {
  ...
  const { error } = await supabase.from("imagem_contraste_registros").insert({
    ...
    lote: txt(formData, "lote") || null, validade: txt(formData, "validade") || null, volume_ml: txt(formData, "volume_ml") || null, via: txt(formData, "via") || null,
    alergia_questionada: true, alergia_negada: formData.get("alergia_negada") === "on", funcao_renal_verificada: formData.get("funcao_renal_verificada") === "on",
    creatinina: txt(formData, "creatinina") || null, egfr: txt(formData, "egfr") || null, consentimento_confirmado: formData.get("consentimento_confirmado") === "on",
