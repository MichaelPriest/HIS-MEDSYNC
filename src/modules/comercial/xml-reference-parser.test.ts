...
describe("xml-reference-parser",()=>{
  it("identifica e importa AMB preservando CH e atributos",()=>{
    ...
    const r=parseCommercialXml(raw,"amb");expect(r.rejeitados).toBe(0);expect(r.itens[0]).toMatchObject({codigo:"10014",pontos_ch:50,quantidade_auxiliares:1,porte:"2",ch_anestesista:10,quantidade_filme:.5,valor_referencia:0});expect(r.itens[0].metadata).toMatchObject({quantidade_aux:1,porte_cirurgico:"2",ch_anestesista:10,quantidade_filme:.5});
  });
  ...
  it("consolida código AMB repetido sem perder a descrição alternativa",()=>{
    const raw='<procedimentos><procedimento><codigoAMB>1</codigoAMB><descricaoAMB>Primeira</descricaoAMB><quantidadeCH>1</quantidadeCH></procedimento><procedimento><codigoAMB>1</codigoAMB><descricaoAMB>Segunda</descricaoAMB><quantidadeCH>1</quantidadeCH></procedimento></procedimentos>';
    const r=parseCommercialXml(raw,"amb");expect(r.itens).toHaveLength(1);expect(r.consolidados).toBe(1);expect(r.itens[0].metadata).toMatchObject({descricoes_alternativas:["Segunda"]});
  });
