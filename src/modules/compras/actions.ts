...
export async function adicionarItemSolicitacaoCompra(formData: FormData) {
  ...
  if (!solicitacaoId || !itemId || quantidade <= 0) {
    redirect(solicitacaoRoute(solicitacaoId || "invalida", "?erro=item"));
  }
  ...
  if (!solicitacao || !["rascunho", "solicitada", "aprovada"].includes(solicitacao.status)) {
    redirect(solicitacaoRoute(solicitacaoId, "?erro=status"));
  }
  if (!item || !["material", "medicamento", "opme", "gas_medicinal"].includes(item.categoria)) {
    redirect(solicitacaoRoute(solicitacaoId, "?erro=item-catalogo"));
  }
  ...
  const { data: existente } = await supabase
    .from("compras_solicitacao_itens")
    .select("id")
...
export async function adicionarFornecedorCotacao(formData: FormData) {
  const { supabase, empresaId } = await requireAnyPermission(["compras.cotar", "compras.gerenciar"]);
  ...
  const { data: fornecedor } = await supabase
    ...
    .eq("ativo", true)
    .maybeSingle();
  if (!fornecedor) redirect(cotacaoRoute(cotacaoId, "?erro=fornecedor"));
  const { data: atual } = await supabase
    .from("compras_cotacao_fornecedores")
    .select("valor_total,itens_cotados,itens_total")
    .eq("cotacao_id", cotacaoId)
    .eq("fornecedor_id", fornecedorId)
    .maybeSingle();
  const { error } = await supabase.from("compras_cotacao_fornecedores").upsert({
    cotacao_id: cotacaoId,
    fornecedor_id: fornecedorId,
    valor_total: Number(atual?.valor_total ?? 0),
    itens_cotados: Number(atual?.itens_cotados ?? 0),
    itens_total: Number(atual?.itens_total ?? 0),
    frete: decimal(text(formData, "frete")),
    prazo_entrega_dias: Number(text(formData, "prazo_entrega_dias") || 0) || null,
    condicao_pagamento: text(formData, "condicao_pagamento") || null,
    observacoes: text(formData, "observacoes") || null,
    atualizado_em: new Date().toISOString(),
  }, { onConflict: "cotacao_id,fornecedor_id" });
  if (error) redirect(cotacaoRoute(cotacaoId, "?erro=fornecedor"));
...
export async function aprovarFornecedorCotacao(formData: FormData) {
  ...
  const { data: proposta } = await supabase
    .from("compras_cotacao_fornecedores")
    .select("id,itens_cotados,itens_total")
    .eq("cotacao_id", cotacaoId)
    .eq("fornecedor_id", fornecedorId)
    .maybeSingle();
  if (!proposta || proposta.itens_total <= 0 || proposta.itens_cotados < proposta.itens_total) {
    redirect(cotacaoRoute(cotacaoId, "?erro=incompleta"));
  }
  const reset = await supabase.from("compras_cotacao_fornecedores").update({ selecionado: false }).eq("cotacao_id", cotacaoId);
  if (reset.error) redirect(cotacaoRoute(cotacaoId, "?erro=aprovar"));
  const select = await supabase.from("compras_cotacao_fornecedores").update({ selecionado: true }).eq("cotacao_id", cotacaoId).eq("fornecedor_id", fornecedorId);
  if (select.error) redirect(cotacaoRoute(cotacaoId, "?erro=aprovar"));
  const quote = await supabase.from("compras_cotacoes").update({ status: "aprovada" }).eq("id", cotacaoId);
  if (quote.error) redirect(cotacaoRoute(cotacaoId, "?erro=aprovar"));
  revalidatePath(`/compras/cotacoes/${cotacaoId}`);
