...
begin
  ...
  if not public.tem_unidade(v_ap.empresa_id,v_ap.unidade_id) then raise exception 'SEM_UNIDADE' using errcode='42501'; end if;
  if not (public.tem_permissao(v_ap.empresa_id,v_ap.unidade_id,'medicamentos.checar_beira_leito') or public.tem_permissao(v_ap.empresa_id,v_ap.unidade_id,'medicamentos.administrar')) then raise exception 'SEM_PERMISSAO' using errcode='42501'; end if;
  ...
  v_paciente_ok := v_codigo<>'' and (
    v_codigo=v_pac.id::text or v_codigo=coalesce(v_pac.ra,'') or v_codigo=coalesce(v_pac.numero_registro::text,'')
  ...
  if p_status='administrado' then
    ...
    if v_disp.estoque_lote_id is not null then select * into v_lote from public.estoque_lotes where id=v_disp.estoque_lote_id; end if;
    ...
    v_medicamento_ok := v_cod_med<>'' and (
      ...
      or v_cod_med=coalesce(v_disp.lote,'')
      or v_cod_med=coalesce(v_lote.numero_lote,'')
      or v_cod_med=v_disp.id::text
    );
  ...
  insert into public.administracoes_medicamentos(
    ...
    case when p_status='administrado' then now() else null end,p_status,coalesce(p_dose,v_p.dose),coalesce(p_via,v_p.via),
    case when p_status='administrado' then v_disp.lote else null end,p_dupla_checagem,p_segundo_profissional_id,
    p_justificativa,auth.uid(),case when p_status='administrado' then v_disp.id else null end,
    ...
    p_codigo_paciente,p_codigo_medicamento,true,case when p_status='administrado' then true else false end,true,true,
    abs(v_atraso)<=v_ap.tolerancia_minutos,v_atraso,v_ap.id
  ) returning id into v_admin;

  update public.prescricao_aprazamentos
  set status=p_status,administracao_id=v_admin,checado_em=now(),justificativa=p_justificativa,updated_at=now(),updated_by=auth.uid()
  where id=v_ap.id;
