...
create table if not exists public.tabelas_procedimentos_edicoes (
  id uuid primary key default gen_random_uuid(),
  fonte_id uuid not null references public.tabelas_procedimentos_fontes on delete cascade,
...
create table if not exists public.tabelas_procedimentos_itens (
  id uuid primary key default gen_random_uuid(),
  edicao_id uuid not null references public.tabelas_procedimentos_edicoes on delete cascade,
  codigo text not null,
  codigo_tuss text,
...
create table if not exists public.contrato_regras_procedimentos (
  ...
  fonte_id uuid not null references public.tabelas_procedimentos_fontes,
  modo_edicao text not null default 'vigente_data' check (modo_edicao in ('vigente_data','edicao_fixa')),
  edicao_fixa_id uuid references public.tabelas_procedimentos_edicoes,
...
alter table public.conta_faturamento_itens add column if not exists metodologia_preco text;
alter table public.conta_faturamento_itens add column if not exists tabela_procedimento_edicao_id uuid references public.tabelas_procedimentos_edicoes;
alter table public.conta_faturamento_itens add column if not exists tabela_procedimento_item_id uuid references public.tabelas_procedimentos_itens;
alter table public.conta_faturamento_itens add column if not exists valor_referencia numeric(14,4);
alter table public.conta_faturamento_itens add column if not exists memoria_calculo jsonb not null default '{}'::jsonb;
...
begin
  ...
  select * into v_regra
  from public.contrato_regras_procedimentos r
  ...
  if p_urgencia and v_regra.aplicar_urgencia then
    v_adicional := v_adicional + coalesce(v_regra.adicional_urgencia_percentual,0);
  end if;
  if p_acomodacao_individual and v_regra.aplicar_acomodacao then
    v_adicional := v_adicional + coalesce(v_regra.adicional_apartamento_percentual,0);
  end if;
  ...
  return query select round(v_final,2), v_fonte.metodologia, v_fonte.id, v_edicao.id, v_item.id,
    jsonb_build_object(
      'base',round(v_base,4),'percentual_ajuste',v_regra.percentual_ajuste,
      'adicional_percentual',v_adicional,'valor_ch_hm',v_regra.valor_ch_hm,
      'valor_ch_sadt',v_regra.valor_ch_sadt,'valor_uco',v_regra.valor_uco,
      'ch_hm',v_item.ch_hm,'ch_sadt',v_item.ch_sadt,'porte',v_item.porte,
      'uco',v_item.uco,'edicao',v_edicao.nome_edicao,'fonte',v_fonte.nome
    );
end;$$;
...
create policy tab_proc_fontes_write on public.tabelas_procedimentos_fontes for all using (public.tem_empresa(empresa_id) and public.tem_permissao(empresa_id,null,'tabelas_procedimentos.gerenciar')) with check (public.tem_empresa(empresa_id) and public.tem_permissao(empresa_id,null,'tabelas_procedimentos.gerenciar'));
create policy tab_proc_edicoes_select on public.tabelas_procedimentos_edicoes for select using (exists(select 1 from public.tabelas_procedimentos_fontes f where f.id=fonte_id and public.tem_empresa(f.empresa_id)));
create policy tab_proc_itens_select on public.tabelas_procedimentos_itens for select using (exists(select 1 from public.tabelas_procedimentos_edicoes e join public.tabelas_procedimentos_fontes f on f.id=e.fonte_id where e.id=edicao_id and public.tem_empresa(f.empresa_id)));
create policy regras_proc_select on public.contrato_regras_procedimentos for select using (exists(select 1 from public.credenciamento_contratos c where c.id=contrato_id and public.tem_empresa(c.empresa_id)));
