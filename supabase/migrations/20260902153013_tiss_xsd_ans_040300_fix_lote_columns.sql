...
begin
  ...
  if jsonb_typeof(v_erros) <> 'array' then
    raise exception 'TISS_XSD_ERROS_INVALIDOS';
  end if;

  if v_hash is not null and v_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'TISS_XSD_HASH_INVALIDO';
  end if;
  ...
  if coalesce(p_xsd_validado,false) and v_hash is null then
    raise exception 'TISS_XSD_HASH_OBRIGATORIO';
  end if;
