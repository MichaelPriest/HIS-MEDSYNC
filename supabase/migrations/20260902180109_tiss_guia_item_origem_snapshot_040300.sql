alter table public.tiss_guia_itens add column if not exists origem_tipo text;
alter table public.tiss_guia_itens add column if not exists conta_item_id uuid references public.conta_faturamento_itens(id) on delete set null;
...
begin
  ...
  if not public.tem_unidade(v_conta.empresa_id,v_conta.unidade_id) or not public.tem_permissao(v_conta.empresa_id,v_conta.unidade_id,'tiss.gerar') then raise exception 'TISS_GUIA_SEM_PERMISSAO' using errcode='42501'; end if;
  if v_conta.tipo_cobranca<>'convenio' or v_conta.convenio_id is null then raise exception 'TISS_CONTA_NAO_CONVENIO'; end if;
  if v_conta.status<>'pronta' or not coalesce(v_conta.auditoria_liberada,false) or not coalesce(v_conta.contas_medicas_liberada,false) then raise exception 'TISS_CONTA_NAO_LIBERADA'; end if;
  ...
  if v_existente is not null then return jsonb_build_object('guia_id',v_existente,'existente',true,'validacao',public.validar_guia_tiss(v_existente)); end if;
  ...
  v_tipo:=case when exists(select 1 from public.internacoes i where i.atendimento_id=v_conta.atendimento_id) then 'resumo_internacao' when v_at.tipo_atendimento_tuss50_codigo='04' then 'consulta' else 'sp_sadt' end;
  ...
  insert into public.tiss_guia_itens(guia_id,sequencial,data_execucao,tabela,codigo_procedimento,descricao,quantidade,valor_unitario,valor_total,reducao_acrescimo,origem_tipo,conta_item_id)
  select v_guia,row_number() over(order by i.data_execucao nulls last,i.created_at,i.id)::integer,
         ...
         round((1+(coalesce(i.percentual_reducao_acrescimo,0)/100.0))::numeric,2),i.origem_tipo,i.id
  ...
  v_validacao:=public.validar_guia_tiss(v_guia);
  ...
  if v_validacao->>'status'='pronta' then perform public.registrar_integracao_evento_internal(v_conta.empresa_id,v_conta.unidade_id,v_conta.atendimento_id,v_conta.paciente_id,'tiss.guia_pronta','tiss_guias',v_guia,now(),jsonb_build_object('conta_id',v_conta.id,'numero_guia_prestador',v_numero)); end if;
