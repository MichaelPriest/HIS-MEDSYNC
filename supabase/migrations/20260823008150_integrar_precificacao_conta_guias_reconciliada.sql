...
create table if not exists public.central_guias_itens (
  ...
  categoria text not null default 'procedimentos',
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname='central_guias_itens_categoria_check') then
    alter table public.central_guias_itens add constraint central_guias_itens_categoria_check check (categoria in ('procedimentos','opme','medicamentos','materiais','taxas','diarias','geral'));
  end if;
  if not exists (select 1 from pg_constraint where conname='central_guias_itens_status_check') then
    alter table public.central_guias_itens add constraint central_guias_itens_status_check check(status in ('solicitado','autorizado','parcial','negado','cancelado'));
  end if;
end $$;

alter table public.central_guias_itens enable row level security;
drop policy if exists central_guias_itens_select on public.central_guias_itens;
