create sequence if not exists public.tiss_lote_numero_seq;

create unique index if not exists financeiro_recebiveis_lote_uidx
  on public.financeiro_recebiveis (lote_id)
...
begin
  ...
  -- Serializa criacoes concorrentes para o mesmo recorte operacional.
  ...
  with elegiveis as (
    select g.id,g.valor_total
     ...
     where g.empresa_id=v_empresa_id
       ...
       and g.status in ('rascunho','pronta')
