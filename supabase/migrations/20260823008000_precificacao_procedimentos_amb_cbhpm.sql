...
-- Amplia as fontes para procedimentos/exames e mantém compatibilidade com OPME/SIMPRO/BRASINDICE.
...
alter table public.tabelas_comerciais_edicoes add column if not exists metodo_calculo text not null default 'fixo'
  check (metodo_calculo in ('fixo','ch_hm_sadt','cbhpm'));
alter table public.tabelas_comerciais_edicoes add column if not exists valor_uco numeric(14,6);
alter table public.tabelas_comerciais_edicoes add column if not exists moeda text not null default 'BRL';

alter table public.tabelas_comerciais_itens add column if not exists pontos_ch numeric(14,6);
alter table public.tabelas_comerciais_itens add column if not exists pontos_hm numeric(14,6);
alter table public.tabelas_comerciais_itens add column if not exists pontos_sadt numeric(14,6);
alter table public.tabelas_comerciais_itens add column if not exists porte text;
alter table public.tabelas_comerciais_itens add column if not exists quantidade_uco numeric(14,6);
alter table public.tabelas_comerciais_itens add column if not exists porte_anestesico text;
alter table public.tabelas_comerciais_itens add column if not exists codigo_auxiliar text;
...
alter table public.contrato_tabelas_comerciais add column if not exists valor_ch numeric(14,6);
alter table public.contrato_tabelas_comerciais add column if not exists valor_hm numeric(14,6);
alter table public.contrato_tabelas_comerciais add column if not exists valor_sadt numeric(14,6);
alter table public.contrato_tabelas_comerciais add column if not exists valor_uco_contratual numeric(14,6);
alter table public.contrato_tabelas_comerciais add column if not exists regras_adicionais jsonb not null default '{}'::jsonb;
alter table public.contrato_tabelas_comerciais add column if not exists arredondamento_casas integer not null default 2 check (arredondamento_casas between 0 and 6);

-- Regras adicionais são contratuais e versionáveis, por exemplo:
-- {"urgencia_percentual":30,"apartamento_percentual":100,"aplicar_urgencia_sobre":"total"}
-- O sistema não impõe horários AMB 90/92; a classificação de horário especial deve ser calculada pela regra contratual/configuração da operadora.
...
alter table public.conta_faturamento_itens add column if not exists memoria_calculo_comercial jsonb;
alter table public.conta_faturamento_itens add column if not exists valor_cobrado_original numeric(14,4);
alter table public.conta_faturamento_itens add column if not exists divergencia_valor_contratual numeric(14,4);
