...
begin
  ...
  if p_item_id is null then
    insert into public.conta_faturamento_itens(
      ...
      quantidade,valor_unitario,percentual_reducao_acrescimo,valor_total,setor,cobravel,observacao,grupo_ato_id,
      ...
      coalesce(nullif(p_payload->>'numero_auxiliares','')::integer,0),coalesce(nullif(p_payload->>'filme_m2','')::numeric,0),
      nullif(p_payload->>'valor_referencia','')::numeric,nullif(p_payload->>'valor_contratual_calculado','')::numeric,
      coalesce(nullif(p_payload->>'origem_valor',''),'lancamento_manual'),nullif(p_payload->>'metodologia_preco',''),
      nullif(p_payload->>'tabela_comercial_edicao_id','')::uuid,nullif(p_payload->>'tabela_comercial_item_id','')::uuid,
