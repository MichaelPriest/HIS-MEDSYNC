...
begin
  ...
  -- No Totem da Recepção o prefixo representa o tipo de atendimento,
  -- evitando que Geral, Preferencial e Emergência sejam todos emitidos como Rxxx.
  v_prefixo := case
    when lower(v_setor.codigo) = 'recepcao' and v_prioridade = 'normal'::public.prioridade_senha then 'G'
    when lower(v_setor.codigo) = 'recepcao' and v_prioridade = 'preferencial'::public.prioridade_senha then 'P'
    when lower(v_setor.codigo) = 'recepcao' and v_prioridade = 'emergencia'::public.prioridade_senha then 'E'
    else upper(coalesce(nullif(trim(v_setor.prefixo),''),'S'))
  end;

  perform pg_advisory_xact_lock(
    hashtextextended(
      p_unidade_id::text || ':' || v_data::text || ':' || v_setor.id::text || ':' || v_prefixo,
      0
    )
  );

  if lower(v_setor.codigo) = 'recepcao' then
  ...
  v_senha := v_prefixo || lpad(v_seq::text,3,'0');
...
'RPC publica do Totem. Na Recepcao, emite prefixos G (geral), P (preferencial) e E (emergencia), cada um com sequencia diaria independente; identificacao por CPF e opcional.';
