...
- O deployment de produção criado para o merge `28b40282...` ficou bloqueado pelo limite externo de builds do Vercel. Portanto, este documento **não declara esse merge publicado em produção** enquanto o deployment correspondente não estiver `READY`.
...
- Pacote atual em desenvolvimento: branch `feat/urgencia-observacao-operacional`, baseada na `main` acima.
...
| Urgência / Emergência | Abertura/encerramento, prioridade, SLA institucional configurável e reavaliação são transacionais. O pacote atual acrescenta Observação operacional no mesmo RA: início/fim rastreáveis, duração, local opcional, destino final e eventos longitudinais. O local de Observação **não cria leito físico**. | validar UX/fluxo com operação real, parametrizar SLA institucional e evoluir protocolos/indicadores locais |
...
## Urgência — Observação operacional (pacote atual)
...
O pacote de Observação **não está homologado** e **não deve ser mesclado** enquanto CI e Vercel não estiverem verdes no mesmo SHA final.
