...
| Internação / NIR | Base funcional em evolução. Painel, mapa de leitos, NIR e Central de Altas têm responsabilidades separadas; alocação revalida disponibilidade e compatibilidade no banco. O PR #81 unifica as duas entradas de admissão no RPC transacional `admitir_internacao_operacional`: sem leito fica `aguardando_leito`; com leito, internação, RA, ocupação e movimentação são atômicos. O fluxo passa a registrar eventos no ledger e divergências na Central. | regras regulatórias, SLA/prioridade, transferências interunidades, geração factual/censo de diárias, giro e homologação presencial com NIR/Internação/Enfermagem |
...
| Auditoria / Contas Médicas | Bases funcionais com hardening de operações sensíveis. O pacote de Faturamento/TISS registra a liberação da auditoria no ledger e exige conta `pronta` + liberações de Auditoria/Contas Médicas antes da Guia TISS transacional. | regras automáticas adicionais, segregação de funções, checklist por convênio e testes de autorização |
| Faturamento / Livro de produção | Base funcional e integrada aos eventos assistenciais/conta. PR #72 registra automaticamente o procedimento cirúrgico e OPME utilizada no livro de produção durante a conclusão e cria/atualiza o grupo do ato cirúrgico quando existe conta aberta compatível. PR #74 passa a sinalizar produção de medicamento incompatível com o desfecho de administração, sem alterar automaticamente o fato faturável. O PR #80 registra material e gás medicinal consumidos no ato cirúrgico pelo tipo canônico do Livro de Produção, usando quantidade líquida após devoluções e origem no movimento físico. O PR #81 preserva a alta clínica mesmo se a preparação de Produção/Conta falhar e converte a falha em pendência crítica `alta_faturamento_nao_preparado`. O pacote atual torna atômica a passagem Conta→Guia TISS e liga lote/protocolo/glosa ao recebível. | completar automações de fechamento, precificação/conta quando aplicável, hardening das policies financeiras e homologação financeira/TISS |
| TISS | Estrutura funcional, ainda não homologada integralmente. Validador anti-glosa, status de guia, requisitos para lote e proteção de itens já existem. O pacote atual elimina criação fragmentada de Guia/Itens e usa RPCs transacionais para Guia, Lote, Protocolo, Glosa e Recurso, com reconciliação derivada. | XSD oficial por versão/tipo, XML definitivo, adapters das operadoras e homologação |
| Glosas / Recursos | Base funcional ampliada no pacote atual: protocolo, glosa e recurso passam por RPCs atômicos; recurso não pode exceder a glosa e o recebível é recalculado pela glosa ativa do lote. | importação automática de demonstrativos, XML definitivo, retorno/deferimento completo e homologação com operadoras |
| Financeiro | Parcialmente integrado ao ciclo da receita. O lote TISS transacional gera recebível; protocolo e glosa recalculam status, glosa e líquido previsto na mesma transação; divergências são apontadas pela Central. | baixas, retenções, conciliação, contas a pagar, caixa e homologação financeira |
...
## Internação ponta a ponta — PR #81

O PR #81 é empilhado sobre o PR #80 e fecha a orquestração de Internação sem criar uma nova fonte clínica:

`Atendimento/RA → admissão transacional → leito/NIR → permanência/prescrição/exames → alta segura → higienização → Produção/Conta → Central de Pendências`.

Migrations efetivamente aplicadas no Supabase conectado e versionadas no branch do PR #81:

- `20260828194919_integracao_internacao_ponta_a_ponta.sql`;
- `20260828202327_internacao_admissao_leito_internal_paridade.sql`.

Regras consolidadas:

- as duas entradas de admissão do frontend chamam o mesmo RPC `admitir_internacao_operacional`;
- sem leito, a internação nasce em `aguardando_leito`; com leito, internação, contexto do RA, ocupação e movimentação são uma única transação;
- atendimento por convênio exige acomodação oficial ANS/TUSS Tabela 49 e preserva código, descrição, versão e canonical do domínio;
- a alocação mantém validações de disponibilidade, reserva, sexo, isolamento e compatibilidade de acomodação;
- o ledger ganha `internacao.admitida`, `leito.alocado`, `leito.transferido`, `internacao.alta` e `leito.higienizacao_concluida`;
- a Central detecta `internacao_sem_leito_status_internado`, `internacao_leito_inconsistente`, `diaria_internacao_sem_producao`, `alta_sem_conta_faturamento`, `alta_com_aprazamento_vencido_pendente`, `alta_sem_higienizacao_leito` e `alta_faturamento_nao_preparado`;
- falha de Produção/Conta depois da alta **não reverte a alta clínica**: a saída assistencial permanece válida e a falha vira pendência crítica rastreável para o Faturamento;
- helpers internos permanecem sem `EXECUTE` para `anon`/`authenticated`; os RPCs operacionais validam autenticação, unidade e RBAC.

A validação funcional foi executada sob role `authenticated` e usuário real com RBAC, em transação com `ROLLBACK`: admissão com Tabela 49 e leito, plano de alta, conciliação medicamentosa, sumário assinado, alta, higienização, preparação de Produção/Conta e reconciliação. Antes do rollback, a conta ficou `pre_faturamento`, o leito em `higienizacao`, os eventos `internacao.admitida → leito.alocado → internacao.alta` estavam registrados e não havia pendência de Internação para o cenário. Após o rollback, ficaram **0 internações, 0 contas e 0 eventos artificiais**, com atendimento e leito restaurados.

Estado real após as migrations: **0 internações ativas, 0 diárias de internação e 0 pendências abertas das novas regras de Internação**. Portanto a validação comprova consistência transacional e RBAC, mas **não equivale a homologação hospitalar presencial**. Permanecem necessários censo/geração factual de diárias, regras regulatórias/SLA, transferências interunidades e validação com NIR, Internação, Enfermagem e Faturamento. O PR #81 continua dependente do #80 e dos gates reais de CI/Vercel.

## Faturamento + TISS + Glosa/Recurso + Financeiro — pacote empilhado no PR #81

Este pacote fecha as transições críticas do ciclo da receita sem criar uma segunda fonte financeira:

`Conta liberada → Guia TISS → Lote → Recebível → Protocolo/Retorno → Glosa → Recurso → Financeiro → Central de Pendências`.

Migrations efetivamente aplicadas no Supabase conectado e versionadas no branch `feat/faturamento-tiss-ponta-a-ponta`:

- `20260828212958_integracao_faturamento_tiss_ponta_a_ponta.sql`;
- `20260828213116_faturamento_tiss_protocolo_glosa_recurso_transacional.sql`.

Regras consolidadas:

- `criar_guia_tiss_conta_transacional` bloqueia conta não conveniada, não liberada ou com críticas impeditivas e cria **Guia + itens** na mesma transação; se já existir guia ativa da conta, retorna a existente de forma idempotente;
- numeração de guia passa a usar sequência do banco, eliminando geração aleatória no frontend;
- a guia é imediatamente submetida ao `validar_guia_tiss_internal`, mantendo as validações existentes de versão, Registro ANS, plano, carteirinha, CNES, profissional/conselho/CBO, TUSS 49/50/52, autorização e itens;
- o lote continua usando `criar_lote_tiss_transacional`, que vincula apenas guias `pronta`, altera o estado das guias e cria `financeiro_recebiveis` atomicamente;
- `registrar_protocolo_tiss_transacional` valida lote/escopo/RBAC e sincroniza protocolo, status e projeção do recebível;
- `registrar_glosa_tiss_transacional` exige que protocolo, guia e item pertençam à mesma cadeia quando informados e recalcula a glosa financeira do lote;
- `criar_recurso_glosa_tiss_transacional` é idempotente para recurso ativo, exige justificativa e bloqueia valor recursado maior que a glosa;
- o ledger ganha fatos de auditoria, criação/prontidão de guia, lote, protocolo, glosa, recurso e recebível;
- a Central detecta conta pronta sem guia, lote sem recebível, guia `em_lote` sem vínculo físico, divergência entre glosa TISS e recebível e glosa marcada em recurso sem item de recurso;
- a reconciliação permanece derivada: não inventa guia, lote, protocolo, glosa, recurso, recebível, baixa ou conciliação para “corrigir” histórico.

Durante o inventário foi encontrado **1 vínculo histórico órfão** em `tiss_lote_guias`, com lote `75000000-0000-4000-8000-000000000002` e guia `75000000-0000-4000-8000-000000000001`; ambos os pais estão ausentes. Como não existe empresa/unidade confiável nesse registro, ele **não foi atribuído artificialmente a nenhum tenant e não foi excluído**. A anomalia é preservada em `integracao_anomalias_globais`, tabela técnica com RLS + FORCE RLS e sem grants para `anon`/`authenticated`.

A validação ponta a ponta foi executada sob role `authenticated` e usuário real com RBAC, dentro de transação revertida. Uma conta histórica de teste foi temporariamente habilitada apenas dentro da transação e percorreu Guia TISS com **2 itens → lote com 1 guia → recebível de R$ 102,50 → protocolo → glosa de R$ 20,00 → recurso de R$ 20,00**. O recebível terminou com líquido previsto de **R$ 82,50**, o ledger registrou os fatos esperados e a reconciliação terminou com **0 pendências de Faturamento** no cenário. Após `ROLLBACK`, ficaram **0 guias, 0 lotes, 0 protocolos, 0 glosas, 0 recursos e 0 recebíveis artificiais**, e a conta retornou ao estado histórico original.

A reconciliação da unidade real após as migrations retornou `abertas_faturamento = 0`; as quatro pendências abertas continuam sendo as divergências históricas de medicamentos já documentadas. O banco segue com **0 guias/lotes/glosas/recursos/recebíveis operacionais reais** fora dos testes revertidos, portanto esta entrega comprova consistência técnica, transacional e de RBAC, mas **não representa homologação TISS/financeira com operadora**. Ainda faltam XSD oficial por versão/tipo, XML definitivo, adapters/retornos reais de operadora, ciclo completo de deferimento/indeferimento, baixas/conciliação financeira e homologação operacional.
