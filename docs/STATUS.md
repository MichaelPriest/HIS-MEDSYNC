...
| Internação / NIR | Base funcional em evolução; PR #16 separa censo por ala/UTI e fila regulatória com sugestão de compatibilidade, PR #17 sincroniza leitos/internações/reservas/bloqueios/higienização via Supabase Realtime e PR #18 especializa o Mapa de Leitos para censo/operação física, deixando reserva e alocação na NIR | compatibilidade obrigatória para isolamento, critérios regulatórios configuráveis, prioridade/SLA, transferências, giro, censo/diárias e alta |
...
O shell continua usando o RBAC apenas para apresentação e descoberta das áreas. O RLS permanece como fronteira definitiva de autorização. O PR #8 adicionou requisitos explícitos de navegação para Urgência/Emergência e NIR. O PR #9 acrescenta a configuração de Estrutura Hospitalar sob `estrutura.visualizar|criar|editar`, com RLS forçado, isolamento por empresa/unidade e sem DELETE pelo cliente. O PR #18 alinha a rota de cadastro físico de leitos à permissão `leitos.gerenciar`; usuários apenas visualizadores não recebem ações operacionais de bloqueio/higienização no Mapa de Leitos.
